...
number of running jobs depending on available system resources.
Thus `ninja` can be started with `-l` and/or `-m` options to prevent
new jobs from starting when load average or memory usage exceed
defined limits.

- `-l` takes a single numeric argument defining the maximum allowed
  Unix load average.
- `-m` accepts either an absolute memory cap (e.g. `-m2000MB`) or a
  percentage (e.g. `-m90%`). In MB mode, Ninja throttles when system-used
  RAM exceeds the specified value. In percentage mode, it throttles when
  memory usage (excluding caches) exceeds the given fraction.
